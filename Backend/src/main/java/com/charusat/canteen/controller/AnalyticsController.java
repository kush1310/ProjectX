...
public class AnalyticsController {
    ...
    private final com.charusat.canteen.repository.PayoutRepository payoutRepository;
    private final org.springframework.jdbc.core.JdbcTemplate jdbc;

    @GetMapping("/vendor/{canteenId}/payouts")
    @org.springframework.security.access.prepost.PreAuthorize("hasAnyRole('CANTEEN_OWNER', 'ADMIN')")
    public ResponseEntity<org.springframework.data.domain.Page<Payout>> getVendorPayouts(
            @PathVariable Long canteenId,
            @org.springframework.data.web.PageableDefault(size = 20) org.springframework.data.domain.Pageable pageable) {
        return ResponseEntity.ok(payoutRepository.findByCanteenId(canteenId, pageable));
    }

    @GetMapping("/vendor/{canteenId}/analytics/export")
    @org.springframework.security.access.prepost.PreAuthorize("hasAnyRole('CANTEEN_OWNER', 'ADMIN')")
    public void exportAnalytics(
        ...
        String filename = "analytics_canteen_" + canteenId + "." + format.toLowerCase();
        response.setHeader("Content-Disposition", "attachment; filename=\"" + filename + "\"");

        response.setContentType("text/csv");
        final java.io.PrintWriter writer = response.getWriter();
        writer.println("OrderID,OrderNumber,Status,PaymentStatus,TotalAmount,CreatedAt");

        jdbc.query("SELECT id, order_number, status, payment_status, total_amount, created_at FROM orders WHERE canteen_id = ? ORDER BY created_at DESC",
                rs -> {
