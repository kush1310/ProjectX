...
public class PasswordResetController {
    ...
    public ResponseEntity<?> forgotPassword(@RequestBody ForgotPasswordRequest request,
        ...
        if (rateLimit.isRateLimited()) {
            ...
            return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS)
                    .body(Map.of(
                            "success", false,
                            "message", "Too many reset requests. Please try again in " + rateLimit.getMinutesRemaining()
                                    + " minutes."));
        }
        ...
        // Send email with the RAW token (the hashed version is stored in DB).
        // The recipient receives the raw token in the reset link, which is then
        // hashed server-side on receipt to look up the stored hash.
        try {
            emailService.sendPasswordResetEmail(user.getEmail(), resetToken.getRawToken());
    ...
    public ResponseEntity<?> validateToken(@RequestParam String token) {
        ...
        // Hash the incoming token to compare against the stored SHA-256 hash in the DB.
        String hashedToken = PasswordResetToken.sha256Hex(token);
    ...
    public ResponseEntity<?> resetPassword(@RequestBody ResetPasswordRequest request, HttpServletRequest httpRequest) {
        ...
        // Validate token — hash the incoming raw token before DB lookup
        String hashedToken = PasswordResetToken.sha256Hex(request.token());
        Optional<PasswordResetToken> tokenOpt = tokenRepository.findByToken(hashedToken);
        ...
        if (!resetToken.isValid()) {
            log.warn("Expired/used reset token attempted for: {} from IP: {}", resetToken.getUser().getEmail(),
                    ipAddress);
        ...
        // Send notification email about password change (OWASP: notify on credential
        // change)
        try {
