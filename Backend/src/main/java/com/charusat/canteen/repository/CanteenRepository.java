        ...
        if (c.getId() == null) {
            ...
            jdbc.update(con -> {
                PreparedStatement ps = con.prepareStatement(
                        "INSERT INTO canteens (name, location, description, image_url, is_open, rush_hour_enabled, opening_time, closing_time, fssai_number, gst_no, bank_name, account_number, ifsc_code, account_holder_name, kyc_document_url, owner_id, created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        Statement.RETURN_GENERATED_KEYS);
                ps.setString(1, c.getName());
                ps.setString(2, c.getLocation());
                ps.setString(3, c.getDescription());
                ps.setString(4, c.getImageUrl());
                ps.setObject(5, c.getIsOpen());
                ps.setObject(6, c.getRushHourEnabled());
                ...
                ps.setTimestamp(17, c.getCreatedAt() != null ? Timestamp.valueOf(c.getCreatedAt()) : null);
                return ps;
            }, keyHolder);
            c.setId(((Number) keyHolder.getKeys().get("id")).longValue());
        } else {
            jdbc.update(
                    "UPDATE canteens SET name=?, location=?, description=?, image_url=?, is_open=?, rush_hour_enabled=?, opening_time=?, closing_time=?, fssai_number=?, gst_no=?, bank_name=?, account_number=?, ifsc_code=?, account_holder_name=?, kyc_document_url=?, owner_id=?, created_at=? WHERE id=?",
                    c.getName(), c.getLocation(), c.getDescription(), c.getImageUrl(), c.getIsOpen(),
