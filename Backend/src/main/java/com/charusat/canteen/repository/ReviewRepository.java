...
public class ReviewRepository {
    ...
    /**
     * findByVendorUserId — returns all reviews for the canteen owned by the given vendor user ID.
     * Resolves the canteen via canteens.owner_id JOIN without requiring the controller to pass canteen_id.
     *
     * @param vendorUserId {Long} - The vendor's user ID (owner_id in canteens table).
     * @returns {List<Review>} — Reviews for the vendor's canteen, newest first.
     */
    public List<Review> findByVendorUserId(Long vendorUserId) {
        String sql = "SELECT r.* FROM reviews r "
                + "INNER JOIN canteens c ON c.id = r.canteen_id "
                + "WHERE c.owner_id = ? "
                + "ORDER BY r.created_at DESC";
        return jdbc.query(sql, ROW_MAPPER, vendorUserId);
    }

    public org.springframework.data.domain.Page<Review> findByVendorUserId(Long vendorUserId, org.springframework.data.domain.Pageable pageable) {
        String countSql = "SELECT COUNT(*) FROM reviews r INNER JOIN canteens c ON c.id = r.canteen_id WHERE c.owner_id = ?";
        Long count = jdbc.queryForObject(countSql, Long.class, vendorUserId);
        long total = count != null ? count : 0;
        String sql = "SELECT r.* FROM reviews r INNER JOIN canteens c ON c.id = r.canteen_id WHERE c.owner_id = ? ORDER BY r.created_at DESC LIMIT ? OFFSET ?";
        List<Review> content = jdbc.query(sql, ROW_MAPPER, vendorUserId, pageable.getPageSize(), pageable.getOffset());
        return new org.springframework.data.domain.PageImpl<>(content, pageable, total);
    }

    public org.springframework.data.domain.Page<Review> findByCustomerId(Long customerId, org.springframework.data.domain.Pageable pageable) {
        Long count = jdbc.queryForObject("SELECT COUNT(*) FROM reviews WHERE customer_id = ?", Long.class, customerId);
        long total = count != null ? count : 0;
        List<Review> content = jdbc.query("SELECT * FROM reviews WHERE customer_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", ROW_MAPPER, customerId, pageable.getPageSize(), pageable.getOffset());
        return new org.springframework.data.domain.PageImpl<>(content, pageable, total);
    }

    public org.springframework.data.domain.Page<Review> findByCanteenId(Long canteenId, org.springframework.data.domain.Pageable pageable) {
        Long count = jdbc.queryForObject("SELECT COUNT(*) FROM reviews WHERE canteen_id = ?", Long.class, canteenId);
        long total = count != null ? count : 0;
        List<Review> content = jdbc.query("SELECT * FROM reviews WHERE canteen_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?", ROW_MAPPER, canteenId, pageable.getPageSize(), pageable.getOffset());
        return new org.springframework.data.domain.PageImpl<>(content, pageable, total);
    }

    public org.springframework.data.domain.Page<Review> findAll(org.springframework.data.domain.Pageable pageable) {
        Long count = jdbc.queryForObject("SELECT COUNT(*) FROM reviews", Long.class);
        long total = count != null ? count : 0;
        List<Review> content = jdbc.query("SELECT * FROM reviews ORDER BY created_at DESC LIMIT ? OFFSET ?", ROW_MAPPER, pageable.getPageSize(), pageable.getOffset());
        return new org.springframework.data.domain.PageImpl<>(content, pageable, total);
    }
}
