...
| **JWT_SECRET** | Backend | Yes | `Cx7MsVDe15ZBjcMeQUzfK3clgDxqN6RfUE9045yLdtE=` | `<256-bit-base64-secret>` | HMAC-SHA256 signature key for JWT tokens. |
| **JWT_EXPIRATION_MS** | Backend | No | `86400000` | `86400000` (24 Hours) | JWT session token validity duration. |
| **FIELD_ENCRYPTION_KEY** | Backend | Yes | `CharusatNeeds2026!AES256FieldKey` | `<32-char-random-key>` | AES-256 key for database sensitive fields. |
| **PAYLOAD_ENCRYPTION_KEY** | Backend | Yes | `CharusatNeedsPayloadKey2026!!` | `<32-char-random-key>` | AES-256-GCM key for in-flight request decryption. |
...
| **RAZORPAY_KEY_ID** | Backend | No | `rzp_test_SYHZJ2Kt3SMiX5` | `rzp_test_SYHZJ2Kt3SMiX5` | Razorpay Merchant Key ID (Test Mode). |
| **RAZORPAY_KEY_SECRET** | Backend | Yes | `kY7BJalMjkmuuP6msNNduBRs` | `kY7BJalMjkmuuP6msNNduBRs` | Razorpay Merchant Secret (Test Mode). |
| **RAZORPAY_WEBHOOK_SECRET** | Backend | Yes | `Ishan@1234` | `<webhook-secret>` | Signature verification key for payment webhooks. |
...
| **B2_KEY_ID** | Backend | Yes | `charusat_admin` | `<backblaze-key-id>` | S3 Access Key ID / Backblaze Key ID. |
| **B2_APPLICATION_KEY** | Backend | Yes | `CharusatMinioPass2026!` | `<backblaze-app-key>` | S3 Secret Key / Backblaze Application Key. |
