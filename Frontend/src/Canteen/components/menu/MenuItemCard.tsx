...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ item, coupons = [], isCategoryOutOfStock = false, onEdit, onDelete, onToggleStatus }) => {
  const [isCouponDropdownOpen, setIsCouponDropdownOpen] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const navigate = useNavigate();

  const isItemAvailable = (item.available ?? item.isAvailable ?? true) && !isCategoryOutOfStock;
  const isDirectlyAvailable = item.available ?? item.isAvailable ?? true;
  ...
  return (
    <motion.div
      ...
      className={`group relative bg-white border rounded-2xl overflow-visible hover:shadow-xl transition-all duration-300 ${
        !isItemAvailable
          ? 'border-red-200 bg-red-50/10'
          : 'border-gray-100'
      } ${isCouponDropdownOpen ? 'z-50' : ''}`}
    >
      <div className="relative p-5">
        {/* Toggle Stock Switch Top Right */}
        <div className="absolute top-4 right-4 z-10">
          <button
            onClick={() => onToggleStatus(item.id, isDirectlyAvailable)}
            className={`px-3 py-1.5 rounded-full text-xs font-extrabold border shadow-sm flex items-center gap-1.5 transition-all cursor-pointer ${
              isDirectlyAvailable && !isCategoryOutOfStock
                ? 'bg-emerald-50 text-emerald-600 border-emerald-200 hover:bg-emerald-100'
                : 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'
            }`}
            title={isDirectlyAvailable ? "Click to mark Out of Stock" : "Click to mark In Stock"}
          >
            {isDirectlyAvailable && !isCategoryOutOfStock ? (
              <>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {!isItemAvailable && (
            <span className="w-fit px-2.5 py-1 bg-red-100 text-red-600 border border-red-200 text-[11px] font-extrabold uppercase tracking-wider rounded-md animate-pulse">
              Out of Stock
            </span>
          )}
          {item.isRecommended && (
          ...
          {(item.tags?.includes('#hidden') || item.isHidden) && (
            <span className="w-fit px-2 py-1 bg-purple-100 text-purple-800 text-[10px] font-bold uppercase tracking-wider rounded-md border border-purple-200">
