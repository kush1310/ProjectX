/**
 * OfferFormRushHour â€” Corporate Professional Design
 * Clean form for RUSH_HOUR coupon type with time window selection.
 */
import React, { useState } from 'react';
import { ArrowLeft, Save, Percent, DollarSign } from 'lucide-react';

interface OfferFormRushHourProps {
...
export const OfferFormRushHour: React.FC<OfferFormRushHourProps> = ({ onSubmit, onBack, initialData }) => {
    const [form, setForm] = useState({
        ...
        rushHourStart: initialData?.rushHourStart || '',
        rushHourEnd: initialData?.rushHourEnd || '',
    ...
    const validate = (): boolean => {
        ...
        if (!form.rushHourStart) errs.rushHourStart = 'Rush hour start is required';
        if (!form.rushHourEnd) errs.rushHourEnd = 'Rush hour end is required';
        if (!form.startTime) errs.startTime = 'Campaign start is required';
        if (!form.endTime) errs.endTime = 'Campaign end is required';
    ...
    const handleSubmit = () => {
        if (!validate()) return;
        onSubmit({
            ...form, couponType: 'RUSH_HOUR', rushHourFlag: true,
        ...
        <div className="max-w-3xl mx-auto px-3 sm:px-6 pb-20">
            ...
            <div className="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
                ...
                <div className="flex-1 min-w-0">
                    <h2 className="text-xl sm:text-2xl font-bold text-gray-900 leading-tight">Create Rush Hour Deal</h2>
                    <p className="text-xs sm:text-sm text-gray-500 mt-0.5 truncate">Time-limited discount during specific hours</p>
                </div>
                <span className="px-2.5 py-1 sm:px-3 sm:py-1.5 bg-pink-50 text-pink-700 text-[10px] sm:text-xs font-bold rounded-md border border-pink-200 uppercase tracking-wide flex-shrink-0">Rush Hour</span>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm">
                ...
                <div className="px-4 sm:px-8 py-5 sm:py-6 space-y-4 sm:space-y-5 border-b border-gray-100">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                        <div>
                            ...
                            <input value={form.couponCode} onChange={e => update('couponCode', e.target.value.toUpperCase())}
                                placeholder="e.g. RUSH50" className={errors.couponCode ? inputErrCls : inputCls} />
                        ...
                        <div>
                            ...
                            <input value={form.title} onChange={e => update('title', e.target.value)}
                                placeholder="e.g. Lunch Rush 50% Off" className={errors.title ? inputErrCls : inputCls} />
                    ...
                    <div>
                        ...
                        <textarea value={form.description} onChange={e => update('description', e.target.value)}
                            rows={2} placeholder="Describe the rush hour deal..."
                            className={`${inputCls} resize-none`} />
                    </div>
                </div>

                {/* Rush Hour Time Window */}
                <div className="px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-100">
                    <h3 className="text-sm sm:text-base font-bold text-gray-800">Rush Hour Time Window</h3>
                    <p className="text-xs sm:text-sm text-gray-400 mt-0.5">The daily time window when this deal is active</p>
                </div>
                <div className="px-4 sm:px-8 py-5 sm:py-6 border-b border-gray-100">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                        <div>
                            ...
                            <input type="time" value={form.rushHourStart} onChange={e => update('rushHourStart', e.target.value)}
                                className={errors.rushHourStart ? inputErrCls : inputCls} />
                            {errors.rushHourStart && <p className="text-sm text-red-500 mt-1.5">{errors.rushHourStart}</p>}
                        </div>
                        <div>
                            <label className={labelCls}>End Time <span className="text-red-500">*</span></label>
                            <input type="time" value={form.rushHourEnd} onChange={e => update('rushHourEnd', e.target.value)}
                                className={errors.rushHourEnd ? inputErrCls : inputCls} />
                            {errors.rushHourEnd && <p className="text-sm text-red-500 mt-1.5">{errors.rushHourEnd}</p>}
                        </div>
                    </div>
                    {form.rushHourStart && form.rushHourEnd && (
                        <div className="mt-4 px-4 py-3 bg-indigo-50 border border-indigo-100 rounded-lg">
                            <p className="text-xs sm:text-sm text-indigo-700 font-bold">
                                Active daily from <strong>{form.rushHourStart}</strong> to <strong>{form.rushHourEnd}</strong>
                            </p>
                        </div>
                    )}
                </div>
                        ...
                        <div>
                            ...
                            <div className="flex border border-gray-300 rounded-lg overflow-hidden">
                                ...
                                <button onClick={() => update('discountType', 'FLAT')}
                                    className={`flex-1 py-3 text-sm font-bold flex items-center justify-center gap-1.5 border-l-2 border-gray-200 transition-colors ${form.discountType === 'FLAT' ? 'bg-indigo-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                ...
                {/* Constraints & Campaign Duration */}
                <div className="px-4 sm:px-8 py-4 sm:py-5 border-b border-gray-100">
                    <h3 className="text-sm sm:text-base font-bold text-gray-800">Constraints & Campaign Duration</h3>
                </div>
                <div className="px-4 sm:px-8 py-5 sm:py-6 space-y-4 sm:space-y-5 border-b border-gray-100">
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5">
                        <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                        <div>
                            <label className={labelCls}>Campaign Start <span className="text-red-500">*</span></label>
                        ...
                        <div>
                            <label className={labelCls}>Campaign End <span className="text-red-500">*</span></label>
                ...
                <div className="px-4 sm:px-8 py-4 sm:py-5 bg-gray-50 border-b border-gray-100">
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-xs sm:text-sm text-gray-600">
                        ...
                        <span><span className="font-bold text-gray-800">Window:</span> {form.rushHourStart && form.rushHourEnd ? `${form.rushHourStart} – ${form.rushHourEnd}` : '—'}</span>
                    </div>
                </div>
...
export default OfferFormRushHour;
