...
export default function CustomerMenuPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        const [menuData, categoriesData, couponsData] = await Promise.all([
          fetchMenu(canteenId),
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 overflow-x-auto no-scrollbar">
          <div className="flex gap-3">
            <button
              ...
              className={`px-5 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-all border ${selectedCategory === 'All'
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-5 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-all border ${selectedCategory === cat.name
                  ? 'bg-rose-500 text-white border-rose-500 shadow-lg shadow-rose-200'
                  : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50 hover:border-gray-300'
                  }`}
              >
              ...
              >
                ...
                <div className="aspect-[4/3] bg-gray-100 relative overflow-hidden">
                  ...
                  {itemDiscounts[item.id] && (
                    <div className="absolute top-12 right-3 bg-red-600 text-white px-2 py-1 rounded-md shadow-lg text-[10px] font-bold uppercase tracking-wide z-10 animate-pulse border border-red-500">
                      {itemDiscounts[item.id].label}
                    </div>
                  )}
                      ...
                      {itemDiscounts[item.id] ? (
                        <div className="flex flex-col items-start">
                          ...
                          <span className="text-xl font-bold text-gray-900">
                            ₹{itemDiscounts[item.id].type === 'PERCENTAGE'
                              ? Math.round(item.price * (1 - itemDiscounts[item.id].value / 100))
                              : Math.max(0, item.price - itemDiscounts[item.id].value)
                            }
                          </span>
                        </div>
                      ) : (
