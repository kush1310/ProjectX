...
def run_seed():
    conn = psycopg2.connect(
        ...
        user="postgres",
        password="postgres",
        dbname="charusatneeds"
    )
    cur = conn.cursor()
    print("Connected to PostgreSQL database charusatneeds.")

    # 1. Truncate all tables
    print("Truncating tables (preserving schema)...")
    cur.execute("""
      TRUNCATE TABLE 
        order_items, 
        orders, 
    ...
    for email, name, role in users:
        cur.execute(
            """INSERT INTO users (email, password, full_name, role, auth_provider, is_active, is_email_verified, created_at)
               VALUES (%s, %s, %s, %s, 'LOCAL', true, true, NOW()) RETURNING id""",
            (email, BCRYPT_HASH, name, role)
        )
        user_ids[email] = cur.fetchone()[0]
        print(f"  User created: {email} (ID: {user_ids[email]})")
    ...
    for name, loc, desc, img, owner_email, op, cl in canteens:
        ...
        cur.execute(
            """INSERT INTO canteens (name, location, description, image_url, is_open, opening_time, closing_time, owner_id, created_at)
               VALUES (%s, %s, %s, %s, true, %s, %s, %s, NOW()) RETURNING id""",
            (name, loc, desc, img, op, cl, owner_id)
        )
        canteen_ids[name] = cur.fetchone()[0]
        print(f"  Canteen created: {name} (ID: {canteen_ids[name]})")

    # Helper function to insert items
    def insert_menu(canteen_name, items):
        cid = canteen_ids[canteen_name]
        inserted = 0
        for item in items:
            name = item[0]
            price = item[1]
            cat = item[2]
            desc = item[3] if len(item) > 3 and item[3] else f"{name} prepared fresh at {canteen_name}."
            rec = item[4] if len(item) > 4 else False

            cur.execute(
                """INSERT INTO menu_items (
                    name, description, price, category, is_available, is_veg, canteen_id, preparation_time, is_recommended, created_at
                ) VALUES (%s, %s, %s, %s, true, true, %s, 15, %s, NOW())""",
                (name, desc, price, cat, cid, rec)
            )
            inserted += 1
        print(f"  Inserted {inserted} items for {canteen_name}")

    # ================== SWEET SPOT ==================
    print("Inserting Sweet Spot items...")
    sweet_spot_items = [
    ...
    insert_menu("Sweet Spot", sweet_spot_items)

    # ================== 99 YOGI ==================
    print("Inserting 99 Yogi items...")
    yogi_items = [
    ...
    insert_menu("99 Yogi", yogi_items)

    # ================== DANNY'S ==================
    print("Inserting Danny's items...")
    dannys_items = [
    ...
    insert_menu("Danny's", dannys_items)

    # ================== ICE BERG ==================
    print("Inserting Ice Berg items...")
    iceberg_items = [
    ...
    insert_menu("Ice Berg", iceberg_items)

    # ================== PRAMUKH ==================
    print("Inserting Pramukh items...")
    pramukh_items = [
    ...
    insert_menu("Pramukh", pramukh_items)

    # ================== GO HUNGER CAFE ==================
    print("Inserting Go Hunger Cafe items...")
    gohunger_items = [
    ...
    insert_menu("Go Hunger Cafe", gohunger_items)

    # ================== PATEL PUFF ==================
    print("Inserting Patel Puff items...")
    patelpuff_items = [
    ...
    insert_menu("Patel Puff", patelpuff_items)

    # 4. Insert Active Coupons
    print("Inserting Active Coupons...")
    coupons = [
        ("WELCOME50", "Welcome Campus Offer", "Flat 50 off on orders above 150", "FLAT", 50, 150, None, "#E23744"),
        ("CHARUSAT20", "Campus Special Discount", "20% off up to 100 on orders above 199", "PERCENTAGE", 20, 199, 100, "#10B981"),
        ("DEALOFTHEDAY", "Deal of the Day", "Flat 100 off on orders above 299", "FLAT", 100, 299, None, "#EA580C"),
        ("PUFFPARTY", "Patel Puff Special", "Flat 20 off on orders above 100", "FLAT", 20, 100, None, "#D97706")
    ]
    ...
    for code, title, desc, dtype, val, min_v, max_c, color in coupons:
        cur.execute(
            """INSERT INTO coupons (
                ...
                gen_random_uuid(), %s, %s, %s, %s, 'GENERAL', %s,
                %s, %s, %s, 1000, 5, 0, true, false, 'COUPON', false, NOW(), NOW()
            )""",
            (code, title, desc, color, dtype, val, min_v, max_c)
        )
        print(f"  Coupon created: {code}")

    # 5. Synchronize categories table
    print("Syncing categories table from menu_items...")
    cur.execute("""
    ...
    print("\n============================================================")
    print("DATABASE SEEDING VERIFIED & COMMITTED!")
    print(f"Total Canteens Created: {canteen_count}")
    print(f"Total Menu Items Created: {item_count}")
    print(f"Total Users Created: {user_count}")
    print(f"Total Coupons Created: {coupon_count}")
    print("============================================================")
